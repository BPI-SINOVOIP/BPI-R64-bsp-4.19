...
//Delete wireless station records.
...
/* 
	...
	1:	forward to bridge
*/
static unsigned char wifi_fwd_check_to_bridge_by_dest_addr(
	struct sk_buff *skb,
	struct net_device *dev)
{
	int i,count=0;
...
static int wifi_fwd_rx_handler(struct sk_buff *skb)
{
	...
	*/
	...
	if ((mh->h_dest[0] & 0x1) == 0x1)
	{
		...
		{
			if (IS_PACKET_FROM_APCLI(recv_from) && 
				!IS_PACKET_FROM_REP_APCLI(recv_from, rep_net_dev)
	...
	/* Handle about lan side staions. We don't want to create this type into pkt_src table. */
	if(wifi_fwd_check_to_bridge_by_dest_addr(skb,net_dev))
		return 1;
...
static void wifi_fwd_func_table_init(struct wifi_fwd_func_table *Fptable) {
	...
	Fptable->wf_fwd_feedback_peer_adapter = wifi_fwd_feedback_peer_adapter;
