...
#define CONFIG_SYS_OS_BASE   0x40000000
#define CONFIG_SYS_SPL_ARGS_ADDR  0x40000000
#define CONFIG_SYS_FDT_BASE  0x40000000	
#define CONFIG_SYS_UBOOT_START 0x41e00000
#define CONFIG_SPL_STACK       0x201000
...
/* Uboot definition */
#define CONFIG_SYS_UBOOT_BASE		        0x30060000    //for spi nor CONFIG_NOR_UBOOT_OFFSET+nor base 0x30000000
...
#define ENV_WRITE_PRELOADER \
	...
	";if test ${filesize_result} = good; then snor erase " __stringify(CONFIG_NOR_PL_OFFSET) " " __stringify(CONFIG_NOR_PL_SIZE) \
	";snor write ${loadaddr} " __stringify(CONFIG_NOR_PL_OFFSET) " 0x40000;fi\0"
#define ENV_WRITE_UBOOT_SPL \
		"wr_uboot_spl=filesize_check " __stringify(CONFIG_NOR_PL_SIZE) \
		";if test ${filesize_result} = good; then snor erase " __stringify(CONFIG_NOR_PL_OFFSET) " " __stringify(CONFIG_NOR_PL_SIZE) \
		";snor write ${loadaddr} " __stringify(CONFIG_NOR_PL_OFFSET) " 0x40000;fi\0"
...
#define ENV_WRITE_UBOOT_SPL \
		"wr_uboot_spl=filesize_check " __stringify(CONFIG_MAX_NAND_PL_SIZE) \
		";if test ${filesize_result} = good; then nand erase " __stringify(CONFIG_NAND_HEADER_OFFSET) " 40000 "\
		";nand write ${loadaddr} " __stringify(CONFIG_NAND_HEADER_OFFSET) " 40000;fi\0"
#define ENV_WRITE_ROM_HEADER \
...
#define ENV_WRITE_PRELOADER \
	"wr_uboot_spl=filesize_check " __stringify(CONFIG_MAX_PL_SIZE*512) \
...
#define ENV_WRITE_UBOOT_SPL \
		"wr_uboot_spl=mmc device 1;mmc write ${loadaddr} " __stringify(CONFIG_SDCARD_PL_BLOCK) \
		" " __stringify(CONFIG_MAX_PL_SIZE) "\0"
#define ENV_WRITE_ROM_HEADER \
...
#define ENV_BOOT_CMD6 \
    "boot6=download_setting uboot_spl;tftpboot ${loadaddr} ${uboot_spl_filename};run wr_uboot_spl\0"
...
#define ENV_BOOT_MENU \
    ...
    "bootmenu_6=7. System Load Uboot_spl then write to Flash via TFTP.=run boot6\0" \
...
#define CONFIG_EXTRA_ENV_SETTINGS \
    ...
    "uboot_spl_filename=uboot_spl.bin\0" \
