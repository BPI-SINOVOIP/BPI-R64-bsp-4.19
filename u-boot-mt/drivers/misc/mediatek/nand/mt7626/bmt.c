...
#include <common.h>
#include <asm/cache.h>
#include <asm/arch-mt7626/nand/bmt.h>
#endif
...
static int load_bmt_data(int start, int pool_size)
{
    ...
    for (bmt_index = start + pool_size - 1; bmt_index >= start; bmt_index--)
    {
		...
		u8 bayi_page[4096];
		u8 bayi_spare[128];
		u8 *page_ptr = bayi_page;
		u8 *spare_ptr = bayi_spare;
			MSG(INIT, "\n <Cheng> Try to write page 0x2001, check for 0xa7\n");
			memset(page_ptr, 0xa9, 4096);
			memset(spare_ptr, 0xFF, 128);
			if (!nand_write_page_bmt(0x2000, page_ptr, spare_ptr))
			{
				MSG(INIT, "Write BMT data fail, need to write again\n");
			}
			memset(dat_buf, 0xFF, 2048);
			MSG(INIT, "\n Try to read bmt from page 0x2001\n");

			if (!nand_read_page_bmt(0x2000, dat_buf, oob_buf))
				{
					 MSG(INIT, "Error found when read page %d\n", PAGE_ADDR(bmt_block_index));
					 //continue;
				}
#endif
...
//}
		...
		/* bayi test for erase verify */
		#if 0
		MSG(INIT, "Erase test here \n");
		for (i = 0; i < 1024; i++) {
			if (!nand_erase_bmt(OFFSET(i)))
            {
                MSG(INIT, "Erase block 0x%x failed\n",  i);
                mark_block_bad_bmt(OFFSET(i));
            }
			MSG(INIT, "Erase block 0x%x success\n", i);
		}
		MSG(INIT, "<Bayi> test Erase block OK\n");
		#endif
        pool_erased = 1;
    }
...
static bool write_bmt_to_flash(u8 * dat, u8 * oob)
{
	...
	#endif
    /* add by bayi */
	if (!nand_read_page_bmt(PAGE_ADDR(bmt_block_index), dat_buf, oob_buf))
    {
         MSG(INIT, "Error found when read page %d\n", PAGE_ADDR(bmt_block_index));
         //continue;
    }
	MSG(INIT, "\n Try to read preloader\n");
	memset(dat_buf, 0xFF, 2048);
	if (!nand_read_page_bmt(0x41, dat_buf, oob_buf))
	{
		MSG(INIT, "Error found when read page %d\n", PAGE_ADDR(bmt_block_index));
		 //continue;
	}
...
bmt_struct *init_bmt(struct nand_chip * chip, int size)
{
	...
	//MSG(INIT, "Bayi-1: : chip->chipsize:0x%llx, bmt size:0x%x \n", chip->chipsize, g_bmt_sz);
#else
