...
IMG_FILE=${U}/${BOARD}-${service}-2k.img
...
(sudo dd if=$ATF	of=${LOOP_DEV} bs=1k seek=512) >/dev/null 2>&1
(sudo dd if=$UBOOT	of=${LOOP_DEV} bs=1k seek=768) >/dev/null 2>&1
...
(dd if=${TMP_FILE} of=${IMG_FILE} bs=1k skip=2 count=1534 status=noxfer) >/dev/null 2>&1
